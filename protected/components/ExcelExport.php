...
class ExcelExport 
{
	...
	public function userClients($arguments)
	{
		...
		$sql='SELECT c.`id_client` , c.`name` as clname, c.`number`, c.`id_city`,ct.`name_city`, cat.`name_category` , p.`name_planning`,f.`name_floor_status`,t.`name_time_status`, c.`price` , s.`name_status`,c.`id_user`,u.`name`, c.`date` 
